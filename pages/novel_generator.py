...
def initialize_session_state():
    return SessionState(novel_generated=False, current_novel="")

def main():
    st.title("소설 생성기 및 수정기")

    if not hasattr(st, "state"):
        st.state = initialize_session_state()
    ...
    if option == "키워드 기반 소설 생성":
        ...
        if st.button("소설 생성", key="keyword_generate"):
            if keywords_input:
                ...
                st.write("생성된 소설:")
                st.write(novel)
                state.novel_generated = True
                state.current_novel = novel
            else:
    ...
    if state.novel_generated:
        st.subheader("기존 소설 수정")
        novel_input = st.text_area("현재 소설:", value=state.current_novel, height=200)
        ...
        if st.button("소설 수정", key="modify_novel"):
            if novel_input and modification_instruction:
                ...
                state.current_novel = modified_novel
            else:
                st.error("수정할 소설과 지시 사항을 모두 입력해주세요.")

        # 생성된 소설 복사 버튼
        if st.button("소설 복사", key="copy_novel"):
            st.text_area("소설 내용 (여기서 복사할 수 있습니다):", value=state.current_novel, height=200)
